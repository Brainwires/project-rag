...
### Testing
```bash
# Run all unit tests (148 tests across all modules)
cargo test --lib
...
2. **MCP Protocol Integration**: Uses `rmcp` macros (`#[tool]`, `#[prompt]`, `#[tool_router]`, `#[prompt_router]`) to define 5 MCP tools and 5 slash commands. The server communicates over stdio following MCP spec.
...
```
src/
├── mcp_server.rs           # Main MCP server with 5 tools + 5 prompts
│   ├── RagMcpServer        # Server state (embedding provider, vector DB, chunker, hash cache)
│   ├── Tool handlers       # index_codebase (smart), query_codebase, get_statistics, etc.
...
- Current test coverage: 148 tests across 9 modules (types, chunker, cache, BM25, embedding, AST parser, file walker)
...
- Tools: Enabled (5 tools available: index_codebase, query_codebase, get_statistics, clear_index, search_by_filters)
- Prompts: Enabled (5 slash commands available: /project:index, /project:query, /project:stats, /project:clear, /project:search)
...
- Run full test suite after updating any dependency (148 tests must pass)
